...
(function () {
  ...
  // Normalise a LaTeX string so the same formula written with slightly
  // different spacing/braces compares equal. Used to match a formula shown on
  // the Formulas page against the "on the formula sheet" set generated by
  // scripts/generate_formula_sheet.py.
  function normalizeLatex(s) {
    var t = String(s == null ? '' : s);
    t = t.replace(/\\tag\{[^}]*\}/g, '');                       // drop equation numbers
    // Drop wrapper / spacing commands that never change a formula's meaning.
    t = t.replace(/\\(left|right|big|Big|bigg|Bigg|boxed|displaystyle|textstyle|mathrm|mathbf|quad|qquad|[,;:!])/g, '');
    t = t.toLowerCase();
    t = t.replace(/[\\\s{}()\[\]_^&]/g, '');                    // strip syntax noise
    return t;
  }
  ...
  window.StudySite = {
    ...
    loadExercises:  function (id) { return fetchJson('../generated/exercises/' + id + '.json'); },
    loadFormulaSheet: function (id) { return fetchJson('../generated/formula_sheet/' + id + '.json'); },
